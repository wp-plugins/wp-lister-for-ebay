 ...
 * ID-based attributes now. Instead, use FindProducts ineBay's Shopping API to 
 * search for product details.<br><br>Retrieves data that you use to construct 
 * valid "product finder" queries(queries against multiple attributes) against 
 * catalog products or (in some cases) listed items.The attributes describe search 
 * criteria (e.g., Manufacturer), as appropriate for the category.Use the results 
 * in combination with GetProductFinderXSL to render the Product Finderin a 
 * graphical user interface.<br><br>GetProductFinder does not conduct the actual 
 * product or listing search.It only returns data about what you can search on. Use 
 * the data as input toGetProductSearchResults to conduct the actual search for 
 * product information.<br><br>To retrieve single-attribute search criteria 
 * (querying against a single attribute, like UPC),use GetProductSearchPage instead 
 * (only applicable for catalog searches).<br><br>See the eBay Web Services Guide 
 * for an overview of Pre-filled Item Information and details aboutsearching for 
